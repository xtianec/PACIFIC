...
require_once __DIR__ . '/../modelos/Linea.php';
...
switch ($op) {
    case 'guardar':
        ...
        $msg = $ok ? 'Línea registrada correctamente' : 'Error al registrar línea';
        echo json_encode(['status' => $ok ? 'success' : 'error', 'msg' => $msg]);
        break;

    case 'editar':
        $ok  = $mc->editar($_POST['id'] ?? 0, $_POST['descripcion'] ?? $_POST['nombre'] ?? '');
        $msg = $ok ? 'Línea actualizada correctamente' : 'Error al actualizar línea';
        echo json_encode(['status' => $ok ? 'success' : 'error', 'msg' => $msg]);
        break;

    case 'desactivar':
        $ok  = $mc->desactivar($_POST['id'] ?? 0);
        $msg = $ok ? 'Línea desactivada' : 'Error al desactivar línea';
        echo json_encode(['status' => $ok ? 'success' : 'error', 'msg' => $msg]);
        break;

    case 'activar':
        $ok  = $mc->activar($_POST['id'] ?? 0);
        $msg = $ok ? 'Línea activada' : 'Error al activar línea';
        echo json_encode(['status' => $ok ? 'success' : 'error', 'msg' => $msg]);
        break;
