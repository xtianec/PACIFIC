<?php
// File: controlador/LineaController.php
    ...
    case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = [
                ...
                "3"=>($reg->is_active??1)
                    ?'<button onclick="editarLinea('.$reg->id.')">✎</button> '
                     .'<button onclick="desactivarLinea('.$reg->id.')">✖</button>'
                    :'<button onclick="activarLinea('.$reg->id.')">✔</button>'
            ];
        }
