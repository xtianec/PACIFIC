...
switch ($op) {
    case 'guardar':
        ...
        $msg = $ok ? 'Unidad de medida registrada correctamente' : 'Error al registrar unidad de medida';
        echo json_encode(['status'=>$ok?'success':'error','msg'=>$msg]);
        break;

    case 'editar':
        $ok  = $mc->editar($_POST['id'] ?? 0, $_POST['descripcion'] ?? $_POST['nombre'] ?? '');
        $msg = $ok ? 'Unidad de medida actualizada correctamente' : 'Error al actualizar unidad de medida';
        echo json_encode(['status'=>$ok?'success':'error','msg'=>$msg]);
        break;

    case 'desactivar':
        $ok  = $mc->desactivar($_POST['id'] ?? 0);
        $msg = $ok ? 'Unidad de medida desactivada' : 'Error al desactivar unidad de medida';
        echo json_encode(['status'=>$ok?'success':'error','msg'=>$msg]);
        break;

    case 'activar':
        $ok  = $mc->activar($_POST['id'] ?? 0);
        $msg = $ok ? 'Unidad de medida activada' : 'Error al activar unidad de medida';
        echo json_encode(['status'=>$ok?'success':'error','msg'=>$msg]);
        break;
