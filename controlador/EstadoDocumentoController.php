<?php
// File: controlador/EstadoDocumentoController.php
require_once __DIR__ . '/../config/Conexion.php';
require_once __DIR__ . '/../modelos/EstadoDocumento.php';

$mc = new EstadoDocumento();
    ...
    case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = [
                ...
                "3"=>($reg->is_active??1)
                    ?'<button onclick="editarEstadoDoc('.$reg->id.')">✎</button> '
                     .'<button onclick="desactivarEstadoDoc('.$reg->id.')">✖</button>'
                    :'<button onclick="activarEstadoDoc('.$reg->id.')">✔</button>'
            ];
        }
