<?php
// File: controlador/RolController.php
    ...
    case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = [
                ...
                "3"=>($reg->is_active??1)
                    ?'<button onclick="editarRol('.$reg->id.')">✎</button> '
                     .'<button onclick="desactivarRol('.$reg->id.')">✖</button>'
                    :'<button onclick="activarRol('.$reg->id.')">✔</button>'
            ];
        }
